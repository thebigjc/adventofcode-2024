...
def solve_part2_parallel(input_data, num_threads=10):
    ...
    chunks = [candidate_cells[i:i+chunk_size] for i in range(0, len(candidate_cells), chunk_size)]

    results = [0]*len(chunks)
    threads = []

    for i, chunk in enumerate(chunks):
        t = threading.Thread(target=worker, args=(grid, start_pos, start_dir, chunk, results, i))
        threads.append(t)
        t.start()
